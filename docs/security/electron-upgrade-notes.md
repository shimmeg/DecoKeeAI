...
- `npm run security:runtime-blockers` now guards the keyboard automation backend decision, active-win executable bit and permission-failure handling, native plugin shell-free launcher, and PowerShell path argument handling.
...
- The electron-builder warning about ad-hoc signing with hardened runtime requiring `com.apple.security.cs.disable-library-validation` was checked against the fresh local `.app`: the main app and helper apps include that entitlement, strict `codesign` verification passes, the app launches, renderer targets appear through CDP, and logs do not show library-validation / `dlopen` / code-signature failures. The current `active-win` runtime warning without Accessibility permission is a separate macOS privacy permission issue, not a signing or library-validation failure.
...
- Packaged `active-win`: the packaging `EACCES` blocker is fixed after making `modules/active-win/main` executable and extending `npm run security:runtime-blockers` to guard the mode bit. The rebuilt bundle contains `Contents/Resources/app/node_modules/active-win/main` as `-rwxr-xr-x`. On this host, without granting the ad-hoc packaged app macOS Accessibility permission, the binary returns `active-win requires the accessibility permission`; the app now treats this as a known limitation and disables repeated active-window lookups for the current session instead of spawning/logging repeatedly.
...
- Runtime warning cleanup: Home Assistant connector startup is skipped until a non-empty valid Home Assistant URL and token are configured, macOS recent-app discovery filters empty Spotlight paths and tolerates per-file lookup failures, and `active-win` monitor paths now validate `owner.path` / window ids before reading them. Foreground-app polling now stays idle until app-monitor profiles are configured, and `active-win` permission failures are logged once and disable further active-window lookups for the current session until the packaged app is restarted with Accessibility permission granted. Assistant window shutdown no longer reads `BrowserWindow` position from the `closed` event after the object has been destroyed.
- Shutdown lifecycle: improved but not complete. `osascript` quit no longer reproduces the assistant-window `Object has been destroyed` exception, but the packaged app can still remain resident after closing windows through the macOS quit path; the final smoke process was stopped manually after log verification.
