...
The blocker was isolated for this runtime spike by removing `robotjs` from the root dependency graph and replacing direct product-source `robotjs` imports with a `keyboardAutomation` adapter. Post-isolation `npm ci`, `npm run rebuild`, Electron `v42.4.1` check, and native load smoke for `node-hid`, `uiohook-napi`, `active-win`, and `sharp` pass on macOS arm64. Keyboard automation now has a `uiohook-napi` backend with a disabled fallback if the native module cannot load; packaged manual smoke with macOS Accessibility permission is still pending.
...
Partially attempted after `robotjs` isolation and repeated after packaged UI smoke fixes. Electron renderer/main bundle compilation and local macOS zip/DMG packaging pass through `npm run buildapp:mac:local` with `electron-builder@26.15.3` and explicit ad-hoc signing. The local `.app` passes `codesign --verify --deep --strict`, and packaged native module load checks pass. The earlier auto-discovered `Apple Development` signing path reached the builder signing step and launched locally, but strict verification now fails immediately after a fresh build, so production Developer ID signing remains pending. Interactive packaged-app startup/restart smoke was run with the built `.app`: the main process and helper/renderer processes start, Accessibility reports the main `DecoKeeAI` window, and CDP sees the main, Settings, AI assistant, icon import, and helper renderer targets. Follow-up UI smoke fixed the packaged resource path bug that hid built-in icons, the packaged `active-win/main` `EACCES` mode bug, fresh English defaults for the main/Settings UI, and Settings close/hide behavior. The latest local package/load smoke confirmed the ad-hoc hardened-runtime build passes strict `codesign` verification; foreground-app polling now stays idle until app-monitor profiles are configured, and without granting Accessibility permission, active-window dependent paths disable repeated lookups for the current session instead of spawning/logging repeatedly. Assistant window shutdown now saves position before close/hide instead of reading a destroyed `BrowserWindow` in `closed`; the broader macOS quit lifecycle can still leave the process resident after windows close and remains a Task 3.4 follow-up. Remaining Task 3.4 gaps are DECOKEE hardware, AI/STT/TTS, plugin runtime, OTA, local server, packaged keyboard automation and active-window Accessibility smoke, quit lifecycle cleanup, and privacy/secrets dynamic checks; they require hardware/API keys/app configuration, user-approved local permissions, or later hardening tasks. Details are recorded in `docs/security/electron-upgrade-notes.md`.
