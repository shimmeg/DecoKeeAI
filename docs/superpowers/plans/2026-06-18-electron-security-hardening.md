...
Partially attempted after `robotjs` isolation. Electron renderer/main bundle compilation passed through `npm run buildapp:mac`. The old nested Electron Builder path failed on missing `/usr/bin/python`; a top-level `electron-builder` override now forces `vue-cli-plugin-electron-builder` to use `electron-builder@26.15.3`, and signed macOS zip/DMG packaging completes. Interactive packaged-app startup/restart smoke was run with the built `.app`: the main process and helper/renderer processes start, Accessibility reports one `DecoKeeAI` window, LaunchServices reports the app as visible, and the user visually confirmed the main window plus Settings window open. Follow-up UI issues remain for globally missing menu icons, Chinese menu labels, and Settings close behavior. DECOKEE hardware, AI/STT/TTS, plugin runtime, OTA, local server, and privacy/secrets dynamic checks were not run; they require hardware/API keys/app configuration or later hardening tasks. Details are recorded in `docs/security/electron-upgrade-notes.md`.
